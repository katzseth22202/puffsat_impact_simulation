# The deep-dish foreclosure is conditional on plane-wave incidence, and inverts under a point source
...
study's geometry (`puffsat_tamper_isp_prd.md`) the precondition fails and the conclusion reverses.
...
concentrates the rebound in the strongest-radiating (`flux ~ v⁸`), `τ≫1` gas and couples radiation
...
**A paraboloid is an optical two-way street.** It focuses *parallel → point* and collimates
*point → parallel*. It cannot do both at once, and which one occurs is set entirely by where the
source sits. In the tamper study the gas arrives from a **compact fireball at finite standoff**, not
as a plane wave. Placed at the focus, that source is *collimated* into a parallel beam and **there is
no focal region at all**. The mechanism that forecloses the deep dish does not exist in this
geometry — not "is outweighed," but is absent.
...
**And the focus-matched optimum lands inside the foreclosed band.** For a paraboloid with its focus
at the fireball, depth-to-diameter is `R/(8d)`: **0.19** at `R = 15 m, d = 10 m` and **0.31** at
`R = 15 m, d = 6 m`, against ADR-0021's shallow band of `d/D ≤ 0.15`.
...
- **Stagnation blunts shape by roughly an order of magnitude.** A hypersonic gas does not reflect; it
  ...
  concave lift — `eta_capture` 0.915 → 0.977 → 0.994 over `d/D` 0 / 0.10 / 0.15, about **+9%** — is
...
- The tamped-nozzle study sweeps flat plus the paraboloid family out to `d/D ≈ 0.35`, and reports the
...
- **Inherit the foreclosure and cap depth at `d/D ≤ 0.15`.** Rejected: off-optimum by construction
