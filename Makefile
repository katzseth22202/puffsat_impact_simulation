...
## analysis-opacity-bracket: grounded opacity uncertainty band on f (Q18/Q10) ->
## data/results/opacity_bracket.csv. Replaces the inherited 0.1x-10x bracket (sized for a
## placeholder Kramers opacity) with the published OPLIB accuracy propagated through the sweep's
...
data/results/seed_window.csv: python/puffsat/conductivity.py python/puffsat/eos_water.py
	@mkdir -p data/results
	PYTHONPATH=python uv run python -m puffsat.conductivity

## analysis-expansion: the cooling history T(t) -- quasi-1D steady isentropic expansion of the
## post-impact plume through the magnetic nozzle, on both branches of the ADR-0026 frozen-
## recombination bracket, with the radiation check and the residence-weighted field leak
## -> data/results/cooling_history.csv
analysis-expansion: data/results/cooling_history.csv

data/results/cooling_history.csv: python/puffsat/expansion.py python/puffsat/eos_water.py \
                                  python/puffsat/conductivity.py python/puffsat/tops.py
	@mkdir -p data/results
	PYTHONPATH=python uv run python -m puffsat.expansion

## analysis-fireball: does recombination freeze below 0.01 kg/m^3? Continues the equilibrium
## expansion past the nozzle lip into the free jet and races atomic three-body recombination
## against it, across a 15-60 deg jet-divergence bracket -> data/results/fireball_freeze.csv
analysis-fireball: data/results/fireball_freeze.csv

data/results/fireball_freeze.csv: python/puffsat/fireball.py python/puffsat/recombination.py \
...
## Committed to the tree (not just gitignored output) because a consumer in another repository
## cannot regenerate it -> data/results/plume_state.csv
analysis-plume: data/results/plume_state.csv

data/results/plume_state.csv: python/puffsat/plume.py python/puffsat/eos_water.py \
                              python/puffsat/expansion.py
	@mkdir -p data/results
	PYTHONPATH=python uv run python -m puffsat.plume
...
## nozzle actually follows. Prints the verdict; owns no artifact.
analysis-recombination:
	PYTHONPATH=python uv run python -m puffsat.recombination
