...
- **Coupled-radiation stability window (found 2026-07-16, shape study; root-caused and FIXED
  2026-07-17).** The 1D coupled runs had a **resolution-onset radiative collapse**: past a
  critical grid refinement that coarsened with `ρv²` (1200 cells at the production ρ = 0.64 at
  16 km/s; ~200–300 cells by ρ ≈ 1.2–1.7), a run died mid-infall with unphysical `e_eff`. An
  instrumented wall-cell trace disproved the initial "radiative over-drain" diagnosis and found
  the real cause: the radiatively-cooled wall cell is compressed past the water table's old
  ρ = 20 kg/m³ ceiling, where the *clamped* `p(ρ)` stops rising and nothing arrests the
  Lagrangian compression (cell width → 0, `dt` → 0, the run stalls) — a table-range defect, not
  a scheme defect. **Fix:** the equilibrium table's density grid is extended node-preservingly
  to 1000 kg/m³ (the first 40 nodes are bit-identical, so results that never hit the ceiling
  reproduce exactly), plus a Newton-iterated backward-Euler matter–radiation exchange hardening
  in the kernel. The onset ladder now completes on the flat plateau at and beyond every
  previously-collapsing resolution, confirming the collapse was numerics, not physics — the
  quoted plateau numbers stand. The shape rows' two-resolution validity protocol is retained as
  a regression guard (`shape_sensitivity.csv` no longer flags any row).
