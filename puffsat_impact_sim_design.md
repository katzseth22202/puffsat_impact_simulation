...
**At low speed the opacity argument fails, and a different one takes over.** At 3.2 km/s the stagnated gas is ~1700 K, neutral, and optically thin. But it is so cold it radiates almost nothing (flux scales ~v^8, so ~5e-5 of the 11 km/s level). The low-v bounce is governed by classical gas-dynamic re-expansion minus condensation/recombination loss. With chemical energy zeroed, the hypothesis was that this is the **worst case in the whole study: water at 3.2 km/s** — if it clears a useful `f`, every other case is easier. **The 1D model refutes the strong form of this (Rung C + B-flux, §10):** 3.2 km/s clears a comfortable `e_eff ≈ 0.74` but is *not* the floor — it bounces *better* than 16 km/s (0.63), because both its loss channels are starved (radiation ~v^8-negligible; condensation/conduction starved by the ~µs bounce). So "if 3.2 km/s clears `f`, everything else does too" does not hold. The worst case is not assumed at an endpoint but **located by measurement at the transitional dip** (ADR-0012, which already flagged the `τ~1` transition as the likely true minimum and the least-certain anchor) — the dense ~5–9 km/s sweep (§6) is where the floor of `f(v)` is actually found.
...
|---|---|---|---|
| `v` | both (anchors) | 3.2-16 km/s | 3 anchors (3.2 / ~8 / 16) + dense 5-9 (transition dip, ADR-0012) |
